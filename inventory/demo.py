...
# (part number, description, category, location, unit, cost, reorder at, opening qty, attributes, interconnect)
PARTS = [
...
def load():
    ...
    for number, name, cat, loc, unit, cost, reorder, qty, values, (family, gender) in PARTS:
        ...
        part, created = Part.objects.get_or_create(part_number=number, defaults={
            ...
            'reorder_level': reorder, 'interconnect_family': family, 'interconnect_gender': gender,
        })
        if not created:
            ...
            if not part.interconnect_family:
                part.interconnect_family, part.interconnect_gender = family, gender
            part.save()
